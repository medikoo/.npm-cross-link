...
const { basename, resolve } = require("path")
    ...
    , log                   = require("log").get("npm-cross-link:user").info
...
module.exports = async ({ path, dependencies }, { packagesPath }, options) => {
	if (dependencies.has("prettier")) return;
	...
	await rm(symbolicLinkPath, { loose: true, recursive: true, force: true });
	await symlink(linkPath, symbolicLinkPath, { type: "junction", intermediate: true });
};
